/*==={ BUILDER }===*/
// Builds all the maps using assets provided by tile.js
// Maps are generated 3-dimensionally and they contain regions
// All tiles within the same region are accessible to each other by movement in 8 directions
...
Game.Builder = function(width, height, depth){
	...
	// Instantiate the arrays to be multi-dimension (3D)
...
Game.Builder.prototype._fillRegion = function(region, x, y, z){
	...
	while (tiles.length > 0){
		...
		// If so, update the region and add it to our processing list.
...
// Remove all tiles of a region at a given depth level - making the wall tiles
...
// Fetches a list of points that overlap between a region at a given depth level, and a region beneath it
Game.Builder.prototype._findRegionOverlaps = function(z, r1, r2){
	var matches = [];
	// Iterate through all tiles, checking if they respect the region constraints and are floor tiles.
	// We check that they are floor to make sure we won't generate two stais on the same tile.
	for (var x = 0; x < this._width; x++){
...
// Try to connect two regions by calculating where they overlap, and add stairs
